    ...
    <div class="container-fluid">
      <div class="row d-flex justify-content-center mt-4">
        <div class="col-md-12 py-5 bg-white">
          <h1 class="text-center color1">Refund</h1>
          <ul class="nav-container bg-light pt-2 px-0">
            ...
            <li
              :class="activeTab == 'Pending' ? 'nav-item active-tab' : 'nav-item'"
              @click="
                () => {
                  activeTab = 'Pending'
                  getRefundOrders()
      ...
      <div class="row mx-2 py-5">
        <!-- ORDER ITEM START -->
        <div class="col-md-6" v-for="order in refundDataDisplay" :key="order.id">
            ...
            <div class="row p-2">
              ...
              <div class="col-md-8 py-2 px-4">
                <div class="d-flex justify-content-end">
                  <small> {{ order.order_created }} </small>
                </div>
                <h3 class="color1">{{ order.name }}</h3>
                <p>
                  Quantity: <b> {{ order.order_quantity }}</b>
                </p>
                <p>Price: {{ order.price }}</p>
                <p>
                  Total Payment: <b>{{ order.payment }}</b>
                </p>
                <div class="d-flex justify-content-end">
                  <!-- <button
                  ...
                  <router-link
                    :to="`/refund/detail/${order.refund_id}`"
                    class="btn btn1"
                    type="button"
...
const refund = ref(computed(() => productStore.refund_orders))
const id = ref('')

const refundDataHolder = ref([])
const refundDataDisplay = ref([])
...
const getPage = (page) => {
  ...
  refundDataDisplay.value = refundDataHolder.value.slice(val1 - 1, val2)
...
const setPagination = () => {
  refundDataHolder.value = refund.value
  refundDataDisplay.value = refundDataHolder.value.slice(0, 4)

  pagination.value.total = refundDataHolder.value.length
  pagination.value.totalPages = parseInt(refundDataHolder.value.length / 4)

  if (refundDataHolder.value.length % 4 !== 0) {
